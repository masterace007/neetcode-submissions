...
select country, year, sum (gold + silver + bronze) as total_medals from olympic_medals where category != 'Gymnastics' group by 
country, year having sum(gold + silver + bronze) > 20 order by sum(gold + silver + bronze) desc limit 5;
